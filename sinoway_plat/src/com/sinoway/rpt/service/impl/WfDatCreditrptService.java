...
public class WfDatCreditrptService implements IWfDatCreditrptService {
	...
	public List<WfDatCreditrpt> query(TableReqParams params, XPeopleInfo curPeople) {
		...
		if (map.get("rptmodenddte").toString() != null && !StringUtils.isBlank(map.get("rptmodenddte").toString())) {
			...
			hql.append(" and rptmoddte <='" + map.get("rptmodenddte").toString().replace("-", "") + "'");
		}
		
		if ("asc".equals(params.getOrderDir())) {
			hql.append(" order by rptmoddte,rptmodtim");
		} else {
			hql.append(" order by rptmoddte desc,rptmodtim desc");
		}
		try {
			list = (List<WfDatCreditrpt>) baseDao.findbyPage(hql.toString(), params.getStartIndex(),params.getPageSize());
	 ...
	 */
	@Transactional(propagation = Propagation.SUPPORTS, readOnly = true, rollbackFor = Exception.class)
	public int count(TableReqParams params,XPeopleInfo curPeople) {
		int result = 0;
		StringBuffer hql = new StringBuffer();
...
//		if (map.get("datcmitori").toString() != null && !"".equals(map.get("datcmitori").toString()) && !StringUtils.isBlank(map.get("datcmitori").toString())) {
//			hql.append(" and datcmitori = '" +map.get("datcmitori").toString() + "'");
//		}
		try {
