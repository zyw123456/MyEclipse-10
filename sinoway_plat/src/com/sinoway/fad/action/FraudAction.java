...
public class FraudAction extends BasePageAction {
	...
	public String findListByConditions() {
		...
		if (StringUtils.isNotBlank(parammap.get("rptid").toString())) {
			param.put(WfDatCreditrpt.CREDITRPT_RPTID, parammap.get("rptid").toString());
		}
		if (StringUtils.isNotBlank(parammap.get("prsncod").toString())) {
			param.put(WfDatCreditrpt.CREDITRPT_PRSNCOD,parammap.get("prsncod").toString());
		}
		if (StringUtils.isNotBlank(parammap.get("prsnnam").toString())) {
			param.put("like_" + WfDatCreditrpt.CREDITRPT_PRSNNAM, "%"+parammap.get("prsnnam").toString()+"%");
		}
		if (StringUtils.isNotBlank(parammap.get("rptmodsrtdte").toString())) {
			param.put("ge_" + WfDatCreditrpt.CREDITRPT_RPTMODDTE, parammap.get("rptmodsrtdte").toString());
		}
		if (StringUtils.isNotBlank(parammap.get("rptmodenddte").toString())) {
			param.put("le_" + WfDatCreditrpt.CREDITRPT_RPTMODDTE, parammap.get("rptmodenddte").toString());
		}
		if(StringUtils.isNotBlank(parammap.get("datcmitori").toString())) {
			param.put(WfDatCreditrpt.CREDITRPT_DATCMITORI, parammap.get("datcmitori").toString());
		}
		// or或者in查询待解决
...
//				RptStatus.STATIS_DELETE.getCode());
		...
		queryBean.setPageNo(params.getStartIndex()/params.getPageSize());
