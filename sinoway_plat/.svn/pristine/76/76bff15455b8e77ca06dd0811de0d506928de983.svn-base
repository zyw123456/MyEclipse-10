...
<script type="text/javascript">
	...
	function summbitMigratePeople(){
		...
		if(originOrgId == targetOrgId){
			top.wfAlert("人员调动目标机构与原始机构相同,不能执行人员调动操作!");
			return;
		}
		var migratePeoples = "";
		for (var i=0; i < count; i++) {
			if(nodes[i].type != "org" && nodes[i].checked){
				if(nodes[i].id == "7B92AE0FC4B04DB48F1AFBDB22CD7188"){
					top.wfAlert("超级管理员不能调动!");
					return;
				}
				if(migratePeoples == ""){
		...
		if(window.confirm('您确认执行人员调动操作?','提示信息')){
			var operAuth=new top.OperAuth();
			operAuth.operType="personMigrate";
			operAuth.authSuccess=function(authUserCode){
				$.post("<%=path%>/peopleAndOrgMigrateAction_summbitMigratePeopleCheck.action", 
				{
				...
				function(data){
					...
					if(dataObj.status == '0000'){
						$.post("<%=path%>/peopleAndOrgMigrateAction_summbitMigratePeople.action", 
							{
								originOrgId:originOrgId,
								targetOrgId:targetOrgId,
								migratePeoples:migratePeoples,
	...
	<table>
		...
		<tr>
			<td colspan="3" align="right">
				<input type="button" onclick="summbitMigratePeople();" value="保存" class="submit_but05" />
