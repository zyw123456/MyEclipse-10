...
(function($) {
	...
	if(typeof settings == "string" ) {
		if(settings == "validate") {
			return !$.validationEngine.submitValidation($(this));
		} else if (settings == "hideAll") {
			$(".formError").each(function(i, d) {
				$.validationEngine.closePrompt($(this),true); 
			});
			return;
	...
	if(settings.inlineValidation == true){ 		// Validating Inline ?
		if(!settings.returnIsValid){					// NEEDED FOR THE SETTING returnIsValid
			...
			if(settings.liveEvent){						// LIVE event, vast performance improvement over BIND
				$(this).find("[class*=validate]:not(:checkbox,:radio)").live(settings.validationEventTriggers, function(caller){ _inlinEvent(this);})
				$(this).find(":checkbox[class*=validate],:radio[class*=validate]").live("click", function(caller){ _inlinEvent(this); })
			}else{
				$(this).find("[class*=validate]:not(:checkbox,:radio)").bind(settings.validationEventTriggers, function(caller){ _inlinEvent(this); })
				$(this).find(":checkbox[class*=validate],:radio[class*=validate]").bind("click", function(caller){ _inlinEvent(this); })
			}
			firstvalid = false;
		}
...
$.validationEngine = {
	...
	loadValidation : function(caller, index) {		// GET VALIDATIONS TO BE EXECUTED
		...
		var validateCalll = $.validationEngine.validateCall(caller,result,index)
		return validateCalll;
	},
	validateCall : function(caller,rules,index) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		var promptText =""	
		
		//if(!$(caller).attr("id")) { $.validationEngine.debug("This field have no ID attribut( name & class displayed): "+$(caller).attr("name")+" "+$(caller).attr("class")) }
		if(!$(caller).attr("id")) {
			$(caller).attr("id", "validAutoID" + $.validationEngine.startID++);
		}
		
		if($(caller).is(":hidden,[disabled],[readonly]")) {
			return;
		}
		...
		if ($.validationEngine.isError == true){
			...
			($("div."+linkTofield).size() ==0) ? $.validationEngine.buildPrompt(caller,promptText,"error", null, index)	: $.validationEngine.updatePromptText(caller,promptText);
		}else{
			$.validationEngine.closePrompt(caller);
		}			
		...
		function _funcCall(caller,rules,position){  		// VALIDATE CUSTOM FUNCTIONS OUTSIDE OF THE ENGINE SCOPE
			...
			if (typeof(fn) === 'function'){
				var msg = fn(caller);
				$.validationEngine.isError = (msg ? true : $.validationEngine.isError);
				if(msg) {
					promptText += msg+"<br />";
				}
			}
			   	...
			   	success: function(data){
			   		if(data == "true"){			// EVERYTING IS FINE, SHOW SUCCESS MESSAGE
			   			...
			   			for(index=0; index<errorNumber; index++){	
			   				...
			   				$.validationEngine.buildPrompt(fieldId,promptError,type,null,index);
		   				}
	   				}
   				}
			})	
	...
	buildPrompt : function(caller,promptText,type,ajaxed,index) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
		if(!$.validationEngine.settings){
			$.validationEngine.defaultSetting()
		}
		...
		if ($.fn.bgiframe && $(caller).is("[bgiframe]")) {
		...
		if($.validationEngine.settings.showOnMouseOver){
			...
			//直接显示错误 2012-10-26
			if(!index || index == 0) {
				$.validationEngine.showTip({data: $(divFormError)});
			}
	...
	submitValidation : function(caller) {					// FORM SUBMIT VALIDATION LOOPING INLINE VALIDATION
		...
		$(caller).find("[class*=validate]").each(function(){
			...
			if(!$("."+linkTofield).hasClass("ajaxed")){	// DO NOT UPDATE ALREADY AJAXED FIELDS (only happen if no normal errors, don't worry)
				var validationPass = $.validationEngine.loadValidation(this, index);
				if(validationPass) {
					index++;
				}
