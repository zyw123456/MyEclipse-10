...
function initPage(){
	...
	$("#logOrgManageList").jqGrid(
		{
			width : pageContentWidth,
			height : tableHeight,
			url : ctx + "/logOrgManage_queryList.action",
			multiselect : false,
			rowNum : 20,
			rownumbers:true,
			rowList : [ 20, 50, 100 ],
			...
			pager : "#logOrgManagePager",
			caption : "机构管理日志信息列表"
		}).trigger("reloadGrid");
	$("#logOrgManageList").navGrid("#logOrgManagePager", {
		edit : false,
		add : false,
		del : false,
		search : false,
 ...
 */
function queryData() {
	$("#logOrgManageList").jqGrid("search", "#queryForm");
}
