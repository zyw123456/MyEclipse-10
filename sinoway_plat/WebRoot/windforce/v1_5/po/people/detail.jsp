	...
	function resetPwd() {
		if (confirm("您确定要重置该用户密码?")) {
			...
			if (top.enableRandomPwdForResetPwd && (top.isNull(email))) {
				...
				$.post(top.ctx + "/resetPwdAction.action?_t="
						+ new Date().getTime(), "userNo=" + currId, function(
						data) {
					wfAlert(data);
				});
			}
	...
	<div style="background-color: #E8EEEB ;height: 100%">
		<table cellspacing="0" width="500px">
			<tr>
				...
				<td width="80%" align="left" class="border_bottom01">
			...
			<tr>
				<td width="20%" class="border_bottom01">身份证号：</td>
				<td width="80%" align="left" class="border_bottom01">
					<s:property value="peopleInfo.peopleIdCard" />
				</td>
			</tr>
			<tr>
				<td width="20%" class="border_bottom01">电话号码：</td>
				<td width="80%" align="left" class="border_bottom01">
					<s:property value="peopleInfo.phone" />
				</td>
			</tr>
			<tr>
				<td width="20%" class="border_bottom01">电子邮箱：</td>
				<td width="80%" align="left" class="border_bottom01">
					<s:property value="peopleInfo.email" />
				</td>
			</tr>
			...
			<tr>
				<td width="20%" class="border_bottom01">用户类型：</td>
				<td width="80%" align="left" class="border_bottom01">
					<s:if test="peopleInfo.peopleType==0">一般用户</s:if>
					<s:if test="peopleInfo.peopleType==1">管理用户</s:if>
				</td>
			</tr>
			<tr>
				<td width="20%" class="border_bottom01">主管级别：</td>
				<td width="80%" align="left" class="border_bottom01">
					<s:if test="peopleInfo.peopleLevel==0">非主管</s:if>
					<s:if test="peopleInfo.peopleLevel==3">主管</s:if>
				</td>
			</tr>
			<tr>
				<td width="20%" class="border_bottom01">岗位列表：</td>
				<td width="80%" align="left" class="border_bottom01">
					<s:iterator value="peopleRoleGroupList" var="roleGroup" status="index">
		...
		<s:if test="poAuthConfig.enableHeadManageBranchGeneralPeople">
			<s:if
				test="poAuthConfig.enableModifiedOwnInfo || (#session.XPEOPLEINFO.sid != #peopleInfo.sid)">
				<%
					if (poCrudList.get(6)) {
				%>
				<div align="right">
					<input type="button" value="重置密码" class="submit_but04" onclick="resetPwd()" />
					&nbsp;&nbsp;
				</div>
				<%
					}
