					...
					if("0".equals(isTradeCode)){
						...
						if("0".equals(getType)){
							if("CLNTJRN".equals(colum)){
								...
								value = StringUtil.NullToString(entity.getCoreHeader().getFnttrndte());// 前置交易日期
							}else if("FRNTTRNTIM".equals(colum)){
								value = StringUtil.NullToString(entity.getCoreHeader().getFnttrntim());// 前置交易时间
						...
						// 从报文体中获取
							...
							if(!"".equals(from)){
								...
								if(deEnCryptFlg && !"".equals(value) && "1".equals(IsDecrypt)){
									try{
										value = DesUtil.getInstance().decrypt(value, entity.getDesKeyInf().getKey(), entity.getDesKeyInf().getVector(), SystemConstant.DEFAULT_CHARSET);
									}catch(Exception e){
								...
								if(!deEnCryptFlg && !"".equals(value) && "1".equals(IsEncrypt)){
									try{
										value = DesUtil.getInstance().encrypt(value, entity.getDesKeyInf().getKey(), entity.getDesKeyInf().getVector(), SystemConstant.DEFAULT_CHARSET);
									}catch(Exception e){
						...
						if(!"0".equals(colum) && "1".equals(assignment)){
							if("CLNTJRN".equals(colum)){
								...
								entity.getCoreHeader().setFnttrndte(value);// 前置交易日期
							}else if("FRNTTRNTIM".equals(colum)){
								entity.getCoreHeader().setFnttrntim(value);// 前置交易时间
							}else if("MASTTRNDTE".equals(colum)){
