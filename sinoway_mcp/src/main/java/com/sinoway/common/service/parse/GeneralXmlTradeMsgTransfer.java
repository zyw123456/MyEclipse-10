...
public class GeneralXmlTradeMsgTransfer {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (extend.equals("0") && node.getChildNodes().getLength() < 1) {
					...
					if(o instanceof JSONObject){
						...
						nodeWithJsonArrayTransferRow(document,sEl,fromArray,rootJson);
					}
	...
	public void  nodeWithJsonArrayTransferRow(Document document,Element el,JSONArray jsa,JSONObject rootJson) throws Exception{
		//获取当前节点的子节点的集合
		NodeList nodes = el.getChildNodes();
