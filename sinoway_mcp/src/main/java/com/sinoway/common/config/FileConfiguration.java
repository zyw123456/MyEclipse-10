...
public class FileConfiguration {
	...
	private void init() {
		...
		String path2 = System.getProperty("user.dir")+File.separator+"config"+configPath;
		System.out.println("Properties path2=--------------------------------"+path2);
		initFiles(path2);
	...
	private void initFiles(String path) {
		...
		if (configdir.exists() && configdir.isDirectory()) {
			for (File comm : configdir.listFiles()) {
				if (comm.isFile() && comm.getName().endsWith("conf")) {
					try {
