...
public class IOStreamUtil {
	...
	public static String Inputstr2Str_Reader(InputStream in, String encode) {
		...
		try {
			if (encode == null || encode.equals("")) {
				// 默认以utf-8形式
				encode = SystemConstant.DEFAULT_CHARSET;
			}
	...
	public static String InputStream2String(InputStream in) throws Exception {
		...
		data = null;
		String res = new String(outStream.toByteArray(), SystemConstant.DEFAULT_CHARSET);
		outStream.close();
		return res;
	}
	...
	public static InputStream Str2Inputstr(String inStr) {
		try {
			return new ByteArrayInputStream(inStr.getBytes(SystemConstant.DEFAULT_CHARSET));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
