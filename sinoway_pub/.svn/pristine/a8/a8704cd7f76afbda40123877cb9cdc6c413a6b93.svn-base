...
public class CommonDefOrganService implements CommonOrganService{
	Logger logger = LogManager.getLogger(CommonDefOrganService.class.getName());
	@Autowired
	...
	try{
		...
		logger.info("新增机构报文"+jo.toJSONString());
		...
		backobj.put("result", msg.toString()); // 返回的描述
		  ...
		  logger.error("新增机构成出现异常"+jo.toJSONString());
		  e.printStackTrace();
		  throw e;
	  }
		return backobj;
	...
	try{
		...
		logger.info("修改机构成报文"+jo.toJSONString());
		BCfgdefOrginfo bCfgdefOrginfo = new BCfgdefOrginfo();
		...
		logger.info("修改机构出现异常"+jo.toJSONString());
		e.printStackTrace();
		throw e;
	}
		return backobj;
	...
	try{
		...
		logger.info("删除机构报文"+jo.toJSONString());
		BCfgdefOrginfo bCfgdefOrginfo = new BCfgdefOrginfo();
		...
		logger.error("删除机构出现异常"+jo.toJSONString());
		e.printStackTrace();
		throw e;
	}
		return backobj;
